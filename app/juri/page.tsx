...
export default function FormPenilaianJuri() {
  ...
  const hitungSkorAkhir = () => {
    ...
    // 1. JURI DLH (Hanya Kategori 1 - Max 40 Poin)
      ...
      <div className="max-w-3xl mx-auto p-4 mt-2 space-y-6">
        ...
        {user.role === "juri_bsi" && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-emerald-100">
            ...
            <div className="space-y-6">
              {/* ✅ BUG FIX: Urutan Usia Pendirian sudah diluruskan! */}
              <ItemSoal id="3.1" judul="1. Usia Pendirian Bank Sampah" skor={skor} onChange={handleInputClick} options={[{l:"< 1 tahun", r:"1-5", min:1, max:5}, {l:"1 - 4 tahun", r:"6-10", min:6, max:10}, {l:"> 5 tahun", r:"11-15", min:11, max:15}]} />
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 shadow-[0_-15px_30px_rgba(0,0,0,0.08)] z-20 flex justify-between items-center px-6 rounded-t-3xl">
        ...
        <div className="flex gap-2 w-1/2">
          ...
          <button 
            ...
            disabled={loading} 
            className="flex-1 bg-slate-900 text-white font-black py-4 rounded-2xl shadow-xl hover:bg-black active:scale-95 disabled:opacity-50 text-sm transition-all"
          >
...
// ... KOMPONEN ItemSoal TETAP SAMA SEPERTI SEBELUMNYA ...
    ...
    <div className="border-b border-slate-100 pb-6 last:border-0 last:pb-0">
      ...
      <div className={`grid gap-3 ${options.length <= 2 ? 'grid-cols-2' : 'grid-cols-2 sm:grid-cols-4'}`}>
        {options.map((opt, i) => {
          ...
          return (
            <button key={i} onClick={() => onChange(id, opt.max)} className={`p-4 rounded-2xl text-left border-2 transition-all flex flex-col justify-between min-h-[95px] active:scale-95 ${isSelected ? "bg-emerald-50 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.2)]" : "bg-white border-slate-100 hover:border-emerald-200 hover:bg-slate-50"}`}>
        ...
        <div className="mt-4 bg-emerald-50 border border-emerald-200 p-5 rounded-2xl flex flex-col gap-4 transition-all animate-fade-in shadow-inner">
          ...
          <div className="flex flex-wrap gap-2.5">
            {Array.from({ length: options[activeIndex].max - options[activeIndex].min + 1 }, (_, i) => options[activeIndex].min + i).map((val) => {
              ...
              return (
                <button key={val} onClick={() => onChange(id, val)} className={`w-14 h-14 flex items-center justify-center rounded-xl text-base font-black transition-all active:scale-90 ${isValSelected ? "bg-emerald-600 text-white shadow-md border-2 border-emerald-700" : "bg-white text-emerald-700 border-2 border-emerald-200 hover:bg-emerald-100"}`}>
                  {val}
                </button>
              );
            })}
